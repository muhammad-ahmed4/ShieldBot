...
export const actions: Actions = {
  default: async ({ request }) => {
    try {
      ...
      const isValid = await TokenService.validatePasswordResetCode(code, email);
      console.log('Password reset code validation result:', isValid);
      
      if (!isValid) {
        console.log('Password reset code validation failed');
        return fail(400, { error: 'Invalid or expired verification code' });
      }

      // Consume the code (delete it)
      const consumed = await TokenService.consumePasswordResetCode(code, email);
